...
class QuantumGUI:
    ...
    def save_simulation_whole(self, path):
        """Function which saves the entire simulation.
        ...
        with open(path+'/sequence.json', 'w') as outfile:
            output = {
                Topology.ALL_NODE: self.graph_to_topology()['nodes'], 
                Topology.ALL_QC_CONNECT: self.graph_to_topology()['qconnections'],
                Topology.ALL_C_CHANNEL+"_table": {"type":"RT", "labels": list(self.cc_delays.columns), "table": self.cc_delays.values.tolist()},
            }
            json.dump(
                output,
                outfile,
    ...
    def get_app(self, name):
        ...
        def run_sim(clicks, n, runtime, units, time_to_run, logging, sim_name):
            ctx = dash.callback_context
            input_id = ctx.triggered[0]['prop_id'].split('.')[0]
            simulation = self.save_simulation_whole(DIRECTORY)
            ...
            if input_id == 'run_sim':
                if time_to_run is None or units is None or sim_name is None:
                    ...
                    network_topo = RouterNetTopo(simulation)
